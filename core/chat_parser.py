...
# Только эти чаты будут отправляться в Discord
ALLOWED_CHATS = ["Общий", "All", "General"]
...
def send_to_discord(channel_id, author, text):
    ...
    if channel:
        future = asyncio.run_coroutine_threadsafe(
            channel.send(f"**{author}** (игра): {text}"),
            bot_instance.loop
        )
...
def process_new_lines():
    ...
    try:
        ...
        print(f"[CHAT] Ошибка чтения {current_chat_file}: {e}")
...
def process_line(line):
    ...
    if match:
        ...
        # Фильтр: только Общий чат
        if chat_name not in ALLOWED_CHATS:
            print(f"[CHAT] Пропущен чат '{chat_name}' от {author}")
            return

        # Не отправляем сообщения от самого бота (из Discord)
        if text.startswith("[Discord]"):
            return

        print(f"[CHAT] Общий чат: {author}: {text}")
...
def check_and_switch_file():
    ...
    if current_chat_file is None or latest != current_chat_file:
        print(f"[CHAT] Переключение на новый чат-файл: {os.path.basename(latest)}")
...
def start_chat_monitoring(bot):
    ...
    if not os.path.exists(settings.PZ_LOG_DIR):
        print(f"[CHAT] Папка логов не существует: {settings.PZ_LOG_DIR}")
        return
    current_chat_file = get_latest_chat_file()
    if current_chat_file is None:
        print("[CHAT] Файл *_chat.txt не найден. Чат-мост отключён.")
        return
    print(f"[CHAT] Отслеживаем файл: {os.path.basename(current_chat_file)}")
    with open(current_chat_file, 'r', encoding='utf-8', errors='ignore') as f:
        f.seek(0, 2)
        file_position = f.tell()
    ...
    observer.start()
    print("[CHAT] Мониторинг чата запущен (watchdog)")
    def periodic_check():
        while True:
            time.sleep(30)
            check_and_switch_file()
...
def stop_chat_monitoring():
    if observer:
        observer.stop()
        observer.join()
        print("[CHAT] Мониторинг чата остановлен")