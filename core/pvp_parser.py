...
def send_kill_to_discord(killer, victim, weapon):
    ...
    if channel:
        msg = f"⚔️ **{killer}** killed **{victim}** using **{weapon}**"
...
def process_new_lines():
    ...
    try:
        ...
        print(f"[PVP] Error reading {current_pvp_file}: {e}")
...
def process_line(line):
    ...
    if kill_match:
        ...
        print(f"[PVP] Kill: {killer} -> {victim} (weapon: {weapon})")
...
def check_and_switch_file():
    ...
    if current_pvp_file is None or latest != current_pvp_file:
        print(f"[PVP] Switching to new pvp file: {os.path.basename(latest)}")
...
def start_pvp_monitoring(bot):
    ...
    if not os.path.exists(settings.PZ_LOG_DIR):
        print("[PVP] Log folder not found")
        return
    current_pvp_file = get_latest_pvp_file()
    if current_pvp_file is None:
        print("[PVP] *_pvp.txt not found. PVP monitoring disabled.")
        return
    print(f"[PVP] Watching file: {os.path.basename(current_pvp_file)}")
    with open(current_pvp_file, 'r', encoding='utf-8', errors='ignore') as f:
        f.seek(0, 2)
        file_position = f.tell()
    ...
    observer.start()
    print("[PVP] PVP monitoring started (watchdog)")
    def periodic_check():
        while True:
            time.sleep(30)
            check_and_switch_file()
...
def stop_pvp_monitoring():
    if observer:
        observer.stop()
        observer.join()
        print("[PVP] PVP monitoring stopped")